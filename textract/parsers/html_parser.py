...
class Parser(BaseParser):
    ...
    _disallowed_names = [
        'style', 'script', '[document]', 'head', 'title', 'html', 'meta', 'link', 'body'
    ]
    ...
    def _inline(self, element):
        """Used to check whether given element can be treated as inline element (without new line after).
        """
                ...
                for tr in trs:
                    ...
                    if len(tds)>0:
                        for i, td in enumerate(tds):
                            ...
                            length = len(td_text)
                            t_dict['col_width'][i] = (max(length, t_dict['col_width'][i]) if (i in t_dict['col_width']) else length)
                            colspan = td.get('colspan')
                            tr_dict.append({'text': td_text, 'colspan': (int(colspan) if colspan!=None else 1)})
    ...
    def _replace_tables(self, soup, v_separator=' | ', h_separator='-'):
        ...
        #Replace <table> elements with raw ASCII
        for t in tables:
            html = ''
            trs = t['trs']
            head_foot = (h_separator * (1 + (v_sep_len * len(t['col_width'])) + t['width'])) + "\n"
            html += head_foot
            for tr in trs:
                html += v_left_sep
